...
const tinkar = new TinkarConfig({
    basePath: (process.env.NEXT_PUBLIC_API_HOST ?? 'https://localhost') + (process.env.NEXT_PUBLIC_API_PORT ?? ':8080') + "/tinkar"
})
tinkar.accessToken = async () => {
    return localStorage.getItem('serviceToken') || '';
}

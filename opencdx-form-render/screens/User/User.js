    ...
    useEffect(() => {
        const fetchQuestionnaireList = async () => {
            try {
                ...
                questionnaires.sort((a, b) => new Date(b.modified).getTime() - new Date(a.modified).getTime());
    ...
    return (
         ...
         Platform.OS === "web" ? (
            ...
            <View style={styles.headerRow}>
              ...
              <Text style={[styles.headerCell2, styles.headerText]}>Last Updated</Text>
            ...
            {buttonTitles.map((questionnaire, index) => (
              <View key={index} style={[styles.row, index % 2 === 0 ? styles.rowEven : styles.rowOdd]}>
                <Text style={[styles.cell, { width: '40%' }]}>{questionnaire.title}</Text> {/* Increased width for first cell */}
                <Text style={styles.cell}>{new Date(questionnaire.modified).toLocaleDateString()}</Text>
                <Text style={styles.cell}>{questionnaire.status}</Text>
                <TouchableOpacity onPress={() => navigation.navigate('ListQuestion', { questionnaire })}>
